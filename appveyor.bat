...
bash -xc "export HB_ARCHITECTURE=win HB_COMPILER=mingw MINGW_INCLUDE=C:\\\\usr\\\\%MSYSTEM%\\\\include; export HB_WITH_CURL=${MINGW_INCLUDE} HB_WITH_OPENSSL=${MINGW_INCLUDE} HB_WITH_PGSQL=${MINGW_INCLUDE} HBMK_WITH_ICU=${MINGW_INCLUDE} HB_INSTALL_PREFIX=C:\\\\projects\\\\harbour-core\\\\harbour HB_VER=${APPVEYOR_REPO_TAG_NAME:=0.0.0}; ./win-make.exe ; ./win-make.exe install"
